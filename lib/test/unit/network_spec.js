...
describe('network', function () {
  ...
  it('read without stream and range', function (done) {
    ...
    var readPromise = Promise.all([_read(), promise]);
  ...
  it('read custom ranges', function (done) {
    ...
    var _read2 = function read(reader, lenResult) {
      return reader.read().then(function (result) {
        if (result.done) {
          return undefined;
        }
        lenResult.value += result.value.byteLength;
        return _read2(reader, lenResult);
      });
    };
    var readPromises = Promise.all([_read2(range1Reader, result1), _read2(range2Reader, result2), promise]);
